...
export class EditPatientComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //fill the current field with the data from api
    this.activatedRoute.paramMap.subscribe((params) => {
      this.patientsService
        .getPatient(Number(params.get('id')))
        .subscribe((value) => {
          this.currentPatient = value;
          this.patientForm.patchValue(value.patient);
          this.assignedGeneralHealthIssuesIDs =
            value.assignedGeneralHealthIssues;
        });
    });
  }
  ...
  currentPatient: TransmittedPatient;
  ...
  checkIssue(issueId: number) {
    return this.assignedGeneralHealthIssuesIDs
  ...
  submit(): void {
    if (this.patientForm.valid) {
      this.patientsService
        .editPatient(
          this.currentPatient.patient.patientID,
